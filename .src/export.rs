...
//! answers every [`ValidationIssue`](crate::ValidationIssue) as a diagnostic;
...
use crate::{Contract, ContractFactory};
use core::ffi::c_void;
    ...
    ($factory:expr, provider = $provider:literal, standard = $standard:literal,
     version = ($major:literal, $minor:literal, $patch:literal)) => {
        ...
        pub unsafe extern "C" fn xmip_create_module_v1(
            ...
            let factory = $factory;
            let identity = $crate::export::Identity {
                provider: $provider,
                standard: $standard,
                version: ($major, $minor, $patch),
            };
            // SAFETY: the host's pointers are passed through unchanged.
            unsafe { $crate::export::create(host, out, factory, identity) }
        }
    };
}
...
mod tests {
    use super::*;
    use crate::{ContractDescriptor, ContractError, ContractId, ValidationIssue, ValidationResult};
